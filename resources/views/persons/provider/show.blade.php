...
@section('content')
    ...
    <div class="content container-fluid">
        <div class="row">
            <div class="col-xs-7">
                <h4 class="page-title">{{__('pages.provider.title_show')}}</h4>
            </div>
            <div class="col-xs-5 text-right m-b-30">
                ...
                <a href="#"
                   ...
                   class="btn btn-danger rounded ">
                    <i class="fa fa-plus"></i>  {{__('pages.provider.details.delete')}}
                </a>
                <a href="{{URL::route('Provider.edit', ['id' => $provider->id]) }}" class="btn btn-primary rounded "><i class="fa fa-plus"></i>
                    {{__('pages.provider.details.edit')}}
                </a>
            </div>
        </div>
                            ...
                            <div class="row">
                                <div class="col-md-5">
                                    <div class="profile-info-left">
                                        ...
                                        @isset($provider->description)<h5 class="company-role m-t-0 m-b-0">{{$provider->description}}</h5>@endisset
                                        @isset($provider->info_box->licence)<div class="staff-id">{{__('pages.provider.details.licence')}} : {{$provider->info_box->licence}}</div>@endisset
                                        @isset($provider->info_box->turnover)<div class="staff-id">{{__('pages.provider.details.turnover')}} : {{$provider->info_box->turnover}}</div>@endisset
                                        @isset($provider->info_box->taxes)<div class="staff-id">{{__('pages.provider.details.taxes')}} : {{$provider->info_box->taxes}}</div>@endisset
                                        @isset($provider->info_box->speaker)<div class="staff-id">{{__('pages.provider.details.speaker')}} : {{$provider->info_box->speaker}}</div>@endisset
                                        <div class="staff-msg"><a href="chat.html" class="btn btn-primary">Send Email</a></div>
                                    </div>
                                </div>
                                <div class="col-md-7">
                                    <ul class="personal-info">
                                        @isset($provider->info_box->tels[0]->tel)
                                            <li>
                                                <span class="title">{{__('pages.client.details.tel')}} :</span>
                                                <span class="text"><a href="#">{{$provider->info_box->tels[0]->tel}}</a></span>
                                            </li>
                                        @endisset
                                        @isset($provider->info_box->fax)
                                            <li>
                                                <span class="title">{{__('pages.client.details.fax')}} :</span>
                                                <span class="text"><a href="#">{{$provider->info_box->fax}}</a></span>
                                            </li>
                                        @endisset
                                        @isset($provider->info_box->emails[0]->email)
                                            <li>
                                                <span class="title">{{__('pages.client.details.email')}} :</span>
                                                <span class="text"><a href="#">{{$provider->info_box->emails[0]->email}}</a></span>
                                            </li>
                                        @endisset

                                        @isset($provider->info_box->address)
                                            <li>
                                                <span class="title">{{__('pages.client.details.address')}} :</span>
                                                <span class="text">{{$provider->info_box->address}}</span>
                                            </li>
                                        @endisset
                                        @isset($provider->info_box->city->city)
                                            <li>
                                                <span class="title">{{__('pages.client.details.city')}} :</span>
                                                <span class="text">{{$provider->info_box->city->city}}</span>
                                            </li>
                                        @endisset
                                        @isset($provider->info_box->zip)
                                            <li>
                                ...
                                <div class="card-box project-box">
                                    <div class="dropdown profile-action">
                                        ...
                                        <ul class="dropdown-menu pull-right">
                                            <li><a href="{{URL::route('Provider.edit',['id'=>$provider->id])}}" data-toggle="modal" data-target="#edit_project"><i class="fa fa-pencil m-r-5"></i> Edit</a></li>
